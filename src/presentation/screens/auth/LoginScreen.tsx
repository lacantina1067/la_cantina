...
const LoginScreen = () => {
  ...
  const floatAnims = useRef(
    BACKGROUND_ICONS.map(() => new Animated.Value(0)),
  ).current;
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (message.includes("Invalid login credentials")) {
        ...
        setError(
          "La cuenta existe, pero falta el perfil en base de datos. Revisa el trigger on_auth_user_created en Supabase.",
        );
      } else {
        setError(
          message ||
            ...
            {isResetEmailSent ? (
              <View style={styles.successContainer}>
                ...
                >
                  ...
                  <Text style={styles.boldWhite}>{resetEmail}</Text>. Revisa tu
                  bandeja de entrada.
                </Text>
                <TouchableOpacity
